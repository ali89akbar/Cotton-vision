"""
Plantwise Actionable Decision Engine - Khairpur, Sindh, Pakistan Context
...
# Khairpur, Sindh Agronomic Rules Matrix for Cotton (Gossypium hirsutum)
KHAIRPUR_AGRONOMIC_RULES: Dict[str, Dict[str, Any]] = {
    "Bacterial Blight": {
        ...
        "field_context_sindh": "Bacterial blight spreads rapidly in hot, humid monsoon weather in Khairpur. Clear infected debris around field boundaries.",
    ...
    "Aphids": {
        ...
        "field_context_sindh": "Aphids cause honeydew secretion and honeydew mold in Khairpur cotton belts during mid-season dry spells.",
        "requires_chemical_spray": True,
    },
    "Army worm": {
        ...
        "field_context_sindh": "Fall Armyworm can cause complete field defoliation within 48 hours in Khairpur. Immediate evening spraying is mandatory.",
        "requires_chemical_spray": True,
    },
    "Powdery Mildew": {
        ...
        "field_context_sindh": "Common during post-monsoon dry weather with heavy morning dew in Khairpur cotton zones.",
        "requires_chemical_spray": True,
    },
    "Target spot": {
        ...
        "field_context_sindh": "Fungal spot promoted by dense plant canopy and lingering humidity in Khairpur fields.",
        "requires_chemical_spray": True,
    },
    "Healthy": {
...
def evaluate_weather_safety(weather_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    Evaluates weather safety for pesticide spraying in Khairpur, Sindh.
    ...
    if wind_kmh > 15.0:
        ...
        warnings.append(
            f"HIGH WIND WARNING ({wind_kmh:.1f} km/h in Khairpur): Postpone chemical spraying due to high risk of spray drift and chemical loss."
        )
    ...
    if temp_c > 40.0:
        ...
        warnings.append(
            f"HEATWAVE WARNING ({temp_c:.1f}°C in Khairpur): High temperatures can burn crop leaves (phytotoxicity). DO NOT spray during peak midday hours."
        )
...
def get_agronomic_advisory(
    ...
    """
    Main decision engine function mapping diagnosis to Khairpur farmer advisory.
    """
    if confidence < confidence_threshold:
        return {
            "status": "LOW_CONFIDENCE",
            "crop": "Cotton (Gossypium hirsutum)",
            "region": "Khairpur, Sindh, Pakistan",
    ...
    matched_class = None
    for class_key in KHAIRPUR_AGRONOMIC_RULES.keys():
        if class_key.lower() in predicted_class.lower() or predicted_class.lower() in class_key.lower():
            matched_class = class_key
            break

    if not matched_class:
        matched_class = predicted_class if predicted_class in KHAIRPUR_AGRONOMIC_RULES else "Healthy"

    rule_info = KHAIRPUR_AGRONOMIC_RULES[matched_class]
    ...
    if rule_info["requires_chemical_spray"] and not weather_assessment["can_spray"]:
        recommendation["actionable_decision"]["spray_advisory_note"] = (
            "CHOSEN CHEMICAL IS IDENTIFIED BUT SPRAYING IS TEMPORARILY POSTPONED DUE TO UNFAVORABLE KHAIRPUR WEATHER CONDITIONS."
        )
